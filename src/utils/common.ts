...
export const sortingNewClients = (data: any, key: any, orderBy: any) => {
  data.sort(function (a: any, b: any) {
    const varA =
      typeof a[0].client[key] === "string"
        ? a[0].client[key].toUpperCase() +
          " " +
          a[0].client["lastName"].toUpperCase()
        : a[0].client[key];
    const varB =
      typeof b[0].client[key] === "string"
        ? b[0].client[key].toUpperCase() +
          " " +
          b[0].client["lastName"].toUpperCase()
        : b[0].client[key];
    let comparison = 0;
